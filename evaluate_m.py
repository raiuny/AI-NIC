...
class Agent:
    def __init__(self, device, actor_lr, critic_lr, hidden_dim_a, hidden_dim_c,
                 state_dim, action_dim, critic_input_dim, gamma, tau) -> None:
        self.net = DDPG(state_dim, action_dim, critic_input_dim,
                     hidden_dim_a,hidden_dim_c, actor_lr, critic_lr, device)
        self.gamma = gamma
        self.tau = tau
        self.critic_criterion = torch.nn.MSELoss()
    ...
    @property
    def actor(self):  # 返回各个actor网络
        return self.net.actor

    @property
    def target_actor(self):  # 返回各个target_actor网络
        return self.net.target_actor
    ...
    def take_action(self, state, explore):
        ...
        #     for i in range(env.agent_number)
        # ]   
        state = torch.tensor(np.array([state], dtype=torch.float, device=self.device))# 读取state,并转变格式
        # return [[
        #     agent.take_action(state, explore)
        #     for agent, state in zip(self.agents, states)
        # ]]
        return self.net.take_action(state, explore)
    def update(self, sample, i_agent):
        '''
        MADDPG更新
        :param sample: 采样的历史数据，包含五项
